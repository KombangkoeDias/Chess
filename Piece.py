import pygame


class ChessPieces:
    def __init__(self, image_file, location):
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load(image_file)
        self.rect = self.image.get_rect()
        self.rect.left, self.rect.top = location

def DrawPawnPieces(screen, PawnList, side):
    for pawnlocation in PawnList:
        if (side == 'white'):
            Pawn = ChessPieces('Assets\Pieces\whitePawn.png', pawnlocation)
            screen.blit(Pawn.image, Pawn.rect)
        else:
            Pawn = ChessPieces('Assets/Pieces/blackPawn.png', pawnlocation)
            screen.blit(Pawn.image, Pawn.rect)

def DrawKnightPieces(screen, KnightList, side):
    for knightlocation in KnightList:
        if (side == 'white'):
            Knight = ChessPieces('Assets\Pieces\whiteKnight.png', knightlocation)
            screen.blit(Knight.image, Knight.rect)
        else:
            Knight = ChessPieces('Assets/Pieces/blackKnight.png', knightlocation)
            screen.blit(Knight.image, Knight.rect)

def DrawBishopPieces(screen, BishopList,side):
    for bishoplocation in BishopList:
        if (side == 'white'):
            Bishop = ChessPieces('Assets\Pieces\whiteBishop.png', bishoplocation)
            screen.blit(Bishop.image, Bishop.rect)
        else:
            Bishop = ChessPieces('Assets/Pieces/blackBishop.png', bishoplocation)
            screen.blit(Bishop.image, Bishop.rect)

def DrawRookPieces(screen, RookList,side):
    for rooklocation in RookList:
        if (side == 'white'):
            Rook = ChessPieces('Assets\Pieces\whiteRook.png', rooklocation)
            screen.blit(Rook.image, Rook.rect)
        else:
            Rook = ChessPieces('Assets/Pieces/blackRook.png', rooklocation)
            screen.blit(Rook.image,Rook.rect)

def DrawKingPieces(screen, KingList,side):
    for kinglocation in KingList:
        if (side == 'white'):
            King = ChessPieces('Assets\Pieces\whiteKing.png', kinglocation)
            screen.blit(King.image, King.rect)
        else:
            King = ChessPieces('Assets\Pieces/blackKing.png', kinglocation)
            screen.blit(King.image,King.rect)

def DrawQueenPieces(screen, QueenList,side):
    for queenlocation in QueenList:
        if (side == 'white'):
            Queen = ChessPieces('Assets\Pieces\whiteQueen.png', queenlocation)
            screen.blit(Queen.image, Queen.rect)
        else:
            Queen = ChessPieces('Assets\Pieces/blackQueen.png', queenlocation)
            screen.blit(Queen.image, Queen.rect)

